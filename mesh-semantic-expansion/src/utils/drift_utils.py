...
__status__ = "Dev"
__desc__ = "Drift utilities for mesh-semantic-expansion: statistical tests, text/embedding stats and Evidently reporting."
'''
    ...
    if "text" in df.columns:
        ...
        if config.feature_engineering.enabled:
            data["avg_token_length"] = text_series.apply(
                lambda t: np.mean([len(w) for w in t.split()]) if t else 0
            )
            logger.debug("Feature engineering drift stats applied")
    ...
    if "embedding" in df.columns:
        ...
        if config.feature_engineering.enabled:
            emb = emb.apply(
                ...
                if x.size and np.linalg.norm(x) > 0
                else x
            )
            logger.debug("Feature engineering drift stats applied")
...
def compute_expansion_stats(df: pd.DataFrame) -> pd.DataFrame:
    """
        Compute semantic expansion statistics

        High-level workflow:
            1) Extract expansions column
            2) Compute number of terms per expansion
            3) Compute average term length
...
def generate_drift_report(
    ...
    html_content = "<html><body><h1>MeSH Drift Report</h1><pre>"
